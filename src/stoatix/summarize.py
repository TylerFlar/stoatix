...
def _select_ok_elapsed(
    iteration_records: list[dict[str, Any]],
) -> float | None:
    """Select the elapsed_s from the first ok attempt for an iteration.
    ...
    Returns:
        elapsed_s of first ok attempt, or None if no ok attempt exists.
    """
    # Sort by attempt to ensure deterministic selection
...
def summarize_results(
    ...
    for (bench_name, case_id, case_key, params_json), case_records in case_groups.items():
        ...
        # For each iteration, select the first ok attempt's elapsed_s
        elapsed_values: list[float] = []
        n_ok = 0
        n_failed = 0

        for iteration in sorted(iteration_groups.keys()):
            elapsed = _select_ok_elapsed(iteration_groups[iteration])
